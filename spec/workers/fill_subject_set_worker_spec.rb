...
RSpec.describe FillSubjectSetWorker do
  let(:panoptes) { instance_double(Panoptes::Client, add_subjects_to_subject_set: true) }
  let(:batch_size) { 5 }
  ...
  before do
    ...
    allow(worker).to receive(:panoptes).and_return(panoptes)
  end

  it 'should add a batch to panoptes' do
    expect(panoptes).to receive(:add_subjects_to_subject_set).with(subject_set_id, (1..batch_size).to_a)
